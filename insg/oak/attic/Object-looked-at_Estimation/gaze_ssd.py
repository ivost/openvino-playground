...
##################################################################
labels_=("aeroplane","bicycle","bird","boat","bottle","bus","car","cat","chair","cow","diningtable","dog","horse","motorbike","person","pottedplant","sheep","sofa","train","tvmonitor")
###################################################################
...
def create_pipeline():
    ...
    ##########################################################################
    # NeuralNetwork
    print("Creating mobilenet-ssd Neural Network...")    
    detection_nn = pipeline.createNeuralNetwork()
    detection_nn.setBlobPath(str((Path(__file__).parent / Path('models/mobilenet-ssd/mobilenet-ssd.blob')).resolve().absolute()))
...
class Main:
    def __init__(self):
        self.device = oak.Device(create_pipeline())
...
#########################################################################       
    def detect_thread(self):
        ...
        while True:
            if self.frame is None:
                continue
            self.det_bboxes = np.array(detection_nn.get().getFirstLayerFp16())
            self.det_bboxes = self.det_bboxes[:np.where(self.det_bboxes == -1)[0][0]]
            self.det_bboxes = self.det_bboxes.reshape((self.det_bboxes.size // 7, 7))
            self.det_bboxes = self.det_bboxes[self.det_bboxes[:, 2] > 0.8][:, 0:7]
########################################################################            
    ...
    def run(self):
        self.threads = [
            ...
            threading.Thread(target=self.detect_thread, daemon=True)###############################################            
        ]
            ...
            if debug:  # face
                ...
                if args.lazer:            
                    ...
                    if self.gaze is not None and self.left_bbox is not None and self.right_bbox is not None:
                        ...
                        cv2.line(self.debug_frame, (le_x, le_y), (le_x + x*100, le_y - y*100), (0, 0, 255), 1)
                        cv2.arrowedLine(self.debug_frame, (re_x, re_y), (re_x + x, re_y - y), (255, 0, 255), 3)
                        cv2.line(self.debug_frame, (re_x, re_y), (re_x + x*100, re_y - y*100), (0, 0, 255), 1)
                        
   ###################################################################################################                     
                    if self.det_bboxes is not None:
                        #print(self.det_bboxes)
                        for raw_det_bbox in self.det_bboxes:
                            label="Label: "
